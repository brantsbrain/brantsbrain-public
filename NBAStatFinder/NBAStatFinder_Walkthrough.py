...
def makeCSV(url, player, season, foldername):
    ...
    with open("soup.html", "w") as writer:
        for character in str(soup):
            try:
                writer.write(character)
            except:
                pass
        pause = input(f"\nPAUSE - Wrote Soup for {player}\n")

    table = soup.findAll("table", {"id":"pgl_basic"})[0]

    with open("table.html", "w") as writer:
        for character in str(table):
            try:
                writer.write(character)
            except:
                pass
    pause = input(f"\nPAUSE - Wrote Table for {player}\n")

    rows = table.findAll("tr")

    with open("rows.html", "w") as writer:
        for character in str(rows):
            try:
                writer.write(character)
            except:
                pass
    pause = input(f"\nPAUSE - Wrote Rows for {player}\n")
    ...
    for a in row.findAll("a"):
        ...
        if "players" in a["href"]:
            try:
                ...
                print(f"{a['href']}")
                print(f"\n{a['href'][:-5]}\n")
                print(f"Browsing to {beginurl}{a['href'][:-5]}/gamelog/{season}")
                pause = input("\nPAUSE\n")
